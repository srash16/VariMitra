...
def test_pairing_logic(client):
    ...
    assert resp.status_code == 404  # Should not find the link anymore (it's now active but pilgrim_device_id doesn't match)

    # Actually, let's check what happens when we try to pair with an active link
    # The pair_complete query looks for is_active = false, so it won't find active links
    resp = client.post("/family/pair/complete", json={
        "pilgrim_device_id": "pilgrim-1",  # Same pilgrim device
        "pairing_code": pairing_code
    })
